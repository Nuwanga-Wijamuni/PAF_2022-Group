...
CustomerManagementServices services = new CustomerManagementServices();
	...
	public String insertCustomerInfo(@FormParam("c_name") String c_name, @FormParam("account_no") String accNo,
								@FormParam("email") String email) 
	{
		
		String output = services.insertCustomerDetails(c_name, account_no,email) ;
		 return output;us_mobile
	}
	...
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	@Produces(MediaType.TEXT_PLAIN)
	public String updateCustomerInfo(@FormParam("c_name") String cus_name, @FormParam("account_no") String account_no,
			@FormParam("email") String email, @FormParam("c_id") String cus_id) {
